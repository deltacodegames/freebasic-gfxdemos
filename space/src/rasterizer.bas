...
#define drawTexturedTrapezoidSigV2U2 sub(a as Vector2, c as Vector2, b as Vector2, d as Vector2, p as Vector2, r as Vector2, q as Vector2, s as Vector2, texture as any ptr)
#define drawTexturedTrapezoidSigV2U3 sub(a as Vector2, c as Vector2, b as Vector2, d as Vector2, p as Vector3, r as Vector3, q as Vector3, s as Vector3, texture as any ptr)

declare sub drawFlatTrapezoid(a as Vector2, c as Vector2, b as Vector2, d as Vector2, colr as integer)
declare sub drawTexturedHorizontalLine_cull   overload(rowStart as any ptr, x0 as integer, x1 as integer, u as Vector2, v as Vector2, image as Image32)
declare sub drawTexturedHorizontalLine_cull   overload(rowStart as any ptr, x0 as integer, x1 as integer, u as Vector3, v as Vector3, image as Image32)
declare sub drawTexturedHorizontalLine_noCull overload(rowStart as any ptr, x0 as integer, x1 as integer, u as Vector2, v as Vector2, image as Image32)
declare sub drawTexturedHorizontalLine_noCull overload(rowStart as any ptr, x0 as integer, x1 as integer, u as Vector3, v as Vector3, image as Image32)
declare sub drawTexturedTrapezoid_cull        overload(a as Vector2, c as Vector2, b as Vector2, d as Vector2, p as Vector2, r as Vector2, q as Vector2, s as Vector2, texture as any ptr)
declare sub drawTexturedTrapezoid_cull        overload(a as Vector2, c as Vector2, b as Vector2, d as Vector2, p as Vector3, r as Vector3, q as Vector3, s as Vector3, texture as any ptr)
declare sub drawTexturedTrapezoid_noCull      overload(a as Vector2, c as Vector2, b as Vector2, d as Vector2, p as Vector2, r as Vector2, q as Vector2, s as Vector2, texture as any ptr)
declare sub drawTexturedTrapezoid_noCull      overload(a as Vector2, c as Vector2, b as Vector2, d as Vector2, p as Vector3, r as Vector3, q as Vector3, s as Vector3, texture as any ptr)
namespace Rasterizer
    declare      sub clipPolyToViewport overload(vertexes() as Vector2, clipped() as Vector2, side as integer = 0)
    declare      sub clipPolyToViewport overload(vertexes() as Vector2, uvs() as Vector2, clippedVerts() as Vector2, clippedUvs() as Vector2, side as integer = 0)
    declare      sub clipPolyToViewport overload(vertexes() as Vector2, uvs() as Vector3, clippedVerts() as Vector2, clippedUvs() as Vector3, side as integer = 0)
...
sub Rasterizer.setPreClipping(doPreClipping as boolean)
    preClipping = doPreClipping
end sub
...
sub Rasterizer.togglePreClipping()
    preClipping = not PreClipping
end sub
private sub Rasterizer.clipPolyToViewport overload(vertexes() as Vector2, clipped() as Vector2, side as integer = 0)
    ...
    if side < 3 then
        clipPolyToViewport newVerts(), clipped(), side + 1
    end if
end sub
sub Rasterizer.clipPolyToViewport overload(vertexes() as Vector2, uvs() as Vector2, clippedVerts() as Vector2, clippedUvs() as Vector2, side as integer = 0)
    ...
    if side < 3 then
        clipPolyToViewport newVerts(), newUvs(), clippedVerts(), clippedUvs(), side + 1
    end if
end sub
sub Rasterizer.clipPolyToViewport overload(vertexes() as Vector2, uvs() as Vector3, clippedVerts() as Vector2, clippedUvs() as Vector3, side as integer = 0)
    dim as Vector2 a, b, c, newVerts(any)
...
sub Rasterizer.drawFlatPoly(vertexes() as Vector2, colr as ulong)
    ...
    clipPolyToViewport vertexes(), clipped()
...
sub Rasterizer.drawTexturedPoly(vertexes() as Vector2, uvs() as Vector2, texture as any ptr)
    ...
    dim as Vector2 clippedVerts(any), clippedUvs(any)
    if preClipping then
        clipPolyToViewport vertexes(), uvs(), clippedVerts(), clippedUvs()
...
sub Rasterizer.drawTexturedPoly(vertexes() as Vector2, uvs() as Vector3, texture as any ptr)
    ...
    dim as Vector3 u, v, w, clippedUvs(any)
    if preClipping then
        clipPolyToViewport vertexes(), uvs(), clippedVerts(), clippedUvs()
        for i as integer = 1 to ubound(clippedVerts)-1
            a = clippedVerts(0)
...
private sub Rasterizer.drawTexturedTri(a as Vector2, b as Vector2, c as Vector2, u as Vector2, v as Vector2, w as Vector2, texture as any ptr)
    dim rasterSub as drawTexturedTrapezoidSigV2U2
    dim as Vector2 d, x
    if preClipping then
        rasterSub = procptr(drawTexturedTrapezoid_noCull, drawTexturedTrapezoidSigV2U2)
    else
        rasterSub = procptr(drawTexturedTrapezoid_cull, drawTexturedTrapezoidSigV2U2)
    end if
    ...
    if a.y < b.y and b.y < c.y then
        ...
        rasterSub(a, a, b, d, u, u, v, x, texture)
        rasterSub(b, d, c, c, v, x, w, w, texture)
    elseif a.y < b.y and b.y = c.y then
        rasterSub(a, a, b, c, u, u, v, w, texture)
    elseif a.y = b.y and b.y < c.y then
        rasterSub(a, b, c, c, u, v, w, w, texture)
    else
        if a.x < b.x then swap a, b
        if a.x < c.x then swap a, c
        if b.x < c.x then swap b, c
...
private sub Rasterizer.drawTexturedTri(a as Vector2, b as Vector2, c as Vector2, u as Vector3, v as Vector3, w as Vector3, texture as any ptr)
    dim rasterSub as drawTexturedTrapezoidSigV2U3
    dim as Vector2 d
    dim as Vector3 x
    if preClipping then
        rasterSub = procptr(drawTexturedTrapezoid_noCull, drawTexturedTrapezoidSigV2U3)
    else
        rasterSub = procptr(drawTexturedTrapezoid_cull, drawTexturedTrapezoidSigV2U3)
    end if
    if a.y > b.y then swap a, b: swap u, v
    if a.y > c.y then swap a, c: swap u, w
    if b.y > c.y then swap b, c: swap v, w
...
private sub drawTexturedHorizontalLine_cull(rowStart as any ptr, x0 as _long_, x1 as _long_, u as Vector2, v as Vector2, image as Image32)
    using Rasterizer
    dim as Vector2 uv, duv
    dim as _long_ cx0, cx1, w
...
private sub drawTexturedHorizontalLine_cull(rowStart as any ptr, x0 as _long_, x1 as _long_, u as Vector3, v as Vector3, image as Image32)
    using Rasterizer
...
private sub drawTexturedHorizontalLine_noCull(rowStart as any ptr, x0 as _long_, x1 as _long_, u as Vector2, v as Vector2, image as Image32)
    using Rasterizer
    dim as Vector2 uv, duv
    dim as ulong ptr pixel
    dim as ulong  w
    ...
    pixel  = rowStart
    pixel += x0
    for x as _long_ = x0 to x1
        *pixel = image.getPixel(uv.x, uv.y)
        pixel += 1
        uv += duv
    next x
end sub
private sub drawTexturedHorizontalLine_noCull(rowStart as any ptr, x0 as _long_, x1 as _long_, u as Vector3, v as Vector3, image as Image32)
    using Rasterizer
    dim as Vector3 uv, duv
    dim as ulong ptr pixel
    dim as ulong  w
...
private sub drawTexturedTrapezoid_cull(a as Vector2, c as Vector2, b as Vector2, d as Vector2, p as Vector2, r as Vector2, q as Vector2, s as Vector2, texture as any ptr)
    using Rasterizer
    dim as Image32 image
    dim as Vector2 uv0, uv1, duv0, duv1
    dim as any ptr buffer, rowStart
    dim as integer ctop, cbtm, h, top, btm
    dim as double  pct, skip, dx0, dx1, x0, x1
    ...
    if image.pixdata <> 0 and buffer <> 0 then
        ...
        skip = ctop - top
        pct  = (0.5 + skip) / h
        x0 = lerpd(a.x, b.x, pct)
        x1 = lerpd(c.x, d.x, pct)
        dx0 = (b.x - a.x) / h
        dx1 = (d.x - c.x) / h
        uv0 = lerp(p, q, pct)
        uv1 = lerp(r, s, pct)
        ...
        for y as integer = ctop to cbtm
            drawTexturedHorizontalLine_cull rowStart, cast(_long_, x0), cast(_long_, x1), uv0, uv1, image
            x0 += dx0
            x1 += dx1
            uv0 += duv0
            uv1 += duv1
...
private sub drawTexturedTrapezoid_cull(a as Vector2, c as Vector2, b as Vector2, d as Vector2, p as Vector3, r as Vector3, q as Vector3, s as Vector3, texture as any ptr)
    using Rasterizer
    dim as Image32 image
...
private sub drawTexturedTrapezoid_noCull(a as Vector2, c as Vector2, b as Vector2, d as Vector2, p as Vector2, r as Vector2, q as Vector2, s as Vector2, texture as any ptr)
    using Rasterizer
    dim as Image32 image
    dim as Vector2 uvl, uvr, duvl, duvr
    dim as any ptr buffer, rowStart
    dim as integer h, top, btm
    dim as double  pct, dxl, dxr, xl, xr
    image.readInfo(texture)
    buffer = screenptr
    if image.pixdata <> 0 and buffer <> 0 then
        if a.x > c.x then swap a, c: swap p, r
        ...
        h   = btm - top + 1
        pct = 0.5 / h
        xl = lerpd(a.x, b.x, pct)
        xr = lerpd(c.x, d.x, pct)
        dxl = (b.x - a.x) / h
        dxr = (d.x - c.x) / h
        uvl = lerp(p, q, pct)
        uvr = lerp(r, s, pct)
        duvl = (q - p) / h
        duvr = (s - r) / h
        rowStart = buffer + top*BUFFER_PITCH
        screenlock
        for y as integer = top to btm
            drawTexturedHorizontalLine_noCull rowStart, cast(_long_, xl), cast(_long_, xr), uvl, uvr, image
            xl += dxl
            xr += dxr
            uvl += duvl
            uvr += duvr
...
private sub drawTexturedTrapezoid_noCull(a as Vector2, c as Vector2, b as Vector2, d as Vector2, p as Vector3, r as Vector3, q as Vector3, s as Vector3, texture as any ptr)
    using Rasterizer
    dim as Image32 image
    dim as Vector3 uvl, uvr, duvl, duvr
    dim as any ptr buffer, rowStart
    dim as integer h, top, btm
    dim as double  pct, dxl, dxr, xl, xr
    image.readInfo(texture)
